...
def render_constraints_tab(result_data):
    ...
    **Note:** Shadow prices from Phase 1 heuristic are rough approximations only.
    ...
    # Display actual shadow prices if available
    shadow_prices = result_data.get('result', {})
    if hasattr(shadow_prices, 'shadow_prices'):
        shadow_prices = shadow_prices.shadow_prices
    else:
        shadow_prices = {}
    
    if shadow_prices:
        st.markdown("**Estimated Shadow Prices:**")
        
        shadow_data = []
        for constraint, value in shadow_prices.items():
            if constraint == 'nox':
                shadow_data.append({
                    'Constraint': 'NOx Allowance',
                    'Shadow Price': f"${value:,.0f}/ton",
                    'Interpretation': f'Additional ton of NOx allowance worth ${value/1e6:.1f}M'
                })
            elif constraint == 'gas':
                shadow_data.append({
                    'Constraint': 'Gas Supply',
                    'Shadow Price': f"${value:,.0f}/MCF-day",
                    'Interpretation': f'Additional MCF/day worth ${value/1e3:.1f}k (NPV)'
                })
            elif constraint == 'land':
                shadow_data.append({
                    'Constraint': 'Land Area',
                    'Shadow Price': f"${value:,.0f}/acre",
                    'Interpretation': f'Additional acre worth ${value/1e3:.1f}k'
                })
        ...
        if shadow_data:
            ...
            st.caption("No binding constraints - shadow prices are zero")
    else:
        st.caption("Shadow prices not calculated for this result")
